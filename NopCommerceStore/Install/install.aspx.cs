...
namespace NopSolutions.NopCommerce.Web.Install
{
    ...
    {
        #region Fields
        private List<string> upgradeableVersions = new List<string>() { "1.10", "1.11", "1.20", "1.30", "1.40" };
        private string newVersion = "1.40";
        #endregion
        ...
        protected void Page_Load(Object sender, EventArgs e)
        {
            ...
            this.radWindowsAuthentication.Text = string.Format("Use integrated Windows authentication (ASP.NET account: {0})", System.Security.Principal.WindowsIdentity.GetCurrent().Name);
        ...
        protected void Page_PreRender(object sender, EventArgs e)
        {
            if (this.Install)
            {
                this.radCreateNew.Enabled = true;
                this.radUseExisting.Enabled = true;
                this.chkCreateSampleData.Enabled = true;
            }
            else
            {
                this.radCreateNew.Enabled = false;
                this.radCreateNew.Checked = false;
                this.radUseExisting.Enabled = true;
                this.radUseExisting.Checked = true;
                this.chkCreateSampleData.Enabled = false;
                this.chkCreateSampleData.Checked = false;
            }
            this.txtPassword.Enabled = this.radSQLAuthentication.Checked;
            this.txtUsername.Enabled = this.radSQLAuthentication.Checked;
            this.txtExistingDatabaseName.Enabled = this.radUseExisting.Checked;
            this.txtNewDatabaseName.Enabled = this.radCreateNew.Checked;
        ...
        protected void OnNextButtonClick(object sender, WizardNavigationEventArgs e)
        {
            if (this.wzdInstaller.ActiveStepIndex == this.wzdInstaller.WizardSteps.IndexOf(this.stpWelcome))
            {
                ...
                this.Install = this.radInstall.Checked;
                #endregion
            }
            else if (this.wzdInstaller.ActiveStepIndex == this.wzdInstaller.WizardSteps.IndexOf(this.stpUserServer))
            {
                ...
                this.TrustedConnection = this.radWindowsAuthentication.Checked;
                ...
                string error = InstallerHelper.TestConnection(this.TrustedConnection, this.txtServerName.Text.Trim(), this.txtExistingDatabaseName.Text.Trim(), this.txtUsername.Text.Trim(), Convert.ToString(ViewState["install.password"]));
                if (!String.IsNullOrEmpty(error))
                {
                    handleError(error);
                    e.Cancel = true;
            ...
            else if (this.wzdInstaller.ActiveStepIndex == this.wzdInstaller.WizardSteps.IndexOf(this.stpDatabase))
            {
                ...
                if (this.radUseExisting.Checked)
                {
                    ...
                    else if (!InstallerHelper.DatabaseExists(this.TrustedConnection, this.txtServerName.Text.Trim(), database, this.txtUsername.Text.Trim(), Convert.ToString(ViewState["install.password"])))
                    {
