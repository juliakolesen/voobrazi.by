...
namespace NopSolutions.NopCommerce.Web.Modules
{
    public partial class PriceRangeFilterControl : BaseNopUserControl
    {
        ...
        {
            List<PriceRange> priceRangeList = GetPriceRangeList();
            if (priceRangeList.Count > 0)
            {
                ...
                else
                {
                    ...
                    string url = CommonHelper.RemoveQueryString(CommonHelper.GetThisPageURL(true), QueryStringProperty);
                    url = excludeQueryStringParams(url);
                    hlRemoveFilter.NavigateUrl = url;

                    lblSelectedPriceRange.Text = getPriceRangeString(SelectedPriceRange.From, SelectedPriceRange.To);
                }
            }
            else
            {
        ...
        protected string getPriceRangeString(decimal? From, decimal? To)
        {
            ...
            if (!From.HasValue)
            {
                ...
                result = string.Format(GetLocaleResourceString("Common.PriceRangeFilter.Under"), string.Format("<span>{0}</span>", toString));
            }
            else if (!To.HasValue)
            {
                string fromString = PriceHelper.FormatPrice(From.Value, true, false);
                result = string.Format(GetLocaleResourceString("Common.PriceRangeFilter.Over"), string.Format("<span>{0}</span>", fromString));
            }
            else
            {
                string fromString = PriceHelper.FormatPrice(From.Value, true, false);
                string toString = PriceHelper.FormatPrice(To.Value, true, false);
                result = string.Format("<span>{0}</span> - <span>{1}</span>", fromString, toString);
            }
            return result;
        }
        ...
        {
            List<PriceRange> _priceRanges = new List<PriceRange>();
            if (string.IsNullOrEmpty(PriceRanges))
                return _priceRanges;
            string[] rangeArray = PriceRanges.Split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        public string QueryStringProperty
        {
            get
            {
                return (((string)ViewState["QueryStringProperty"]) ?? "Price");
            }
            set
            {
                ViewState["QueryStringProperty"] = value;
            }
        }
        ...
        public PriceRange SelectedPriceRange
        {
            get
            {
                string _range = CommonHelper.QueryString(QueryStringProperty);
                ...
                if (_fromTo.Length == 2)
                {
                    ...
                    List<PriceRange> priceRangeList = GetPriceRangeList();
                    foreach (PriceRange pr in priceRangeList)
                    {
