...
namespace NopSolutions.NopCommerce.BusinessLogic.CustomerManagement
{
    ...
    /// </summary>
    public partial class DeleteExpiresCustomerSessionsTask : ITask
    {
        ...
        public void Execute(XmlNode node)
        {
            ...
            CustomerSessionCollection customerSessions = CustomerManager.GetAllCustomerSessions();
            foreach (CustomerSession customerSession in customerSessions)
            {
                if (customerSession.LastAccessed.AddMinutes((double)this._deleteExpiredCustomerSessionsOlderThanMinutes) < DateTime.UtcNow)
                {
                    //may be marking as expired is better way ???
                    CustomerManager.DeleteCustomerSession(customerSession.CustomerSessionGUID);
                }
            }
        }
    }
